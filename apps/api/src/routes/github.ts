...
githubRouter.get("/callback", async (c) => {
  ...
  const secret =
    ...
    "development_secret_key_1234567890_vigil_app";
  ...
  try {
    ...
    if (nonceRes.rowCount === 0) {
      return c.text(
        "Bad Request: OAuth state expired, invalid, or already consumed",
        400,
      );
    }
    ...
    const clientSecret =
      process.env.GITHUB_CLIENT_SECRET || "placeholder_secret";
    ...
    if (!tokenRes.ok) {
      throw new Error(
        `GitHub token exchange responded with status ${tokenRes.status}`,
      );
    }
    ...
    if (!tokenData.access_token) {
      throw new Error(
        tokenData.error || "Access token missing in GitHub response",
      );
    }
    ...
    return c.redirect(
      `${dashboardUrl}/settings?project_id=${payload.projectId}&connected=github`,
    );
  } catch (err: any) {
...
githubRouter.get("/connect", async (c) => {
  ...
  if (!projectId) {
    return c.json(
      { ok: false, success: false, error: "Missing projectId" },
      400,
    );
  }
  ...
  await pool.query(`DELETE FROM oauth_states WHERE expires_at < $1`, [
    Date.now() - 3600000,
  ]);
  ...
  const secret =
    process.env.BETTER_AUTH_SECRET ||
    "development_secret_key_1234567890_vigil_app";
  const stateToken = jwt.sign({ nonce, projectId, userId }, secret, {
    expiresIn: "10m",
  });
...
githubRouter.get("/repos", async (c) => {
  ...
  if (!conn) {
    return c.json(
      { ok: false, success: false, error: "Integration not connected" },
      400,
    );
  }
...
githubRouter.post(
  "/select-repo",
  zValidator("json", selectRepoSchema),
  async (c) => {
    const {
      projectId,
      repoOwner,
...
githubRouter.post("/disconnect", async (c) => {
  ...
  await pool.query(`DELETE FROM github_connections WHERE project_id = $1`, [
    projectId,
  ]);
  ...
  async (c) => {
    ...
    await pool.query(
     ...
     SET github_auto_raise_enabled = $1,
         ...
         triage_model = COALESCE($5, triage_model)
     WHERE id = $6`,
      [
        autoRaiseEnabled,
        autoRaiseSeverity,
      ...
      return c.json(
        {
          ok: false,
          success: false,
          error: "An issue is already raising or linked to this group",
        },
        409,
      );
    }
    return c.json(
      {
        ok: false,
        success: false,
        error: err.message || "Failed to raise issue on GitHub",
      },
      err.status || 500,
    );
  }
